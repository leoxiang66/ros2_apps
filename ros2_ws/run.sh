...
list_nodes() {
  ...
  if [ -d "$package_path/$package_name" ]; then
    for file in $(find "$package_path/$package_name" -type f -name '*.py' ! -name '__init__.py'); do
      nodes+=("$(basename "$file" .py)")
    done
  elif [ -d "$package_path/src" ]; then
    for file in $(find "$package_path/src" -type f -name '*.cpp'); do
      nodes+=("$(basename "$file" .cpp)")
    done
  fi
  echo "${nodes[@]}"
}
